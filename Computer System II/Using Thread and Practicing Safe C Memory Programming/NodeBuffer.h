...
class   NodeBuffer
{
  ...
  int   outIndex_;
  int   numItems_;
  pthread_mutex_t	lockIt_;//mutex means preventing multi threads reads/write the same variable
  pthread_cond_t	 notEmpty_;
  pthread_cond_t 	notFull_;

public :

  NodeBuffer        ()//this is constructor
  {
	...
	pthread_mutex_init(&lockIt_, NULL);//initialize variable "lockit_ "here
	pthread_cond_init(&notEmpty_, NULL);
	pthread_cond_init(&notFull_, NULL);
  }

  ~NodeBuffer       ()//destructor, run when u get rid of the objects
  {
